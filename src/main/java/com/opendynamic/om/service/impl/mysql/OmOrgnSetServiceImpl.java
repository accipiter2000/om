...
public class OmOrgnSetServiceImpl implements OmOrgnSetService {
    ...
    public int copyOrgnSet(String BASE_ORGN_SET_ID_, final String ORGN_SET_ID_, Date UPDATE_DATE_, String OPERATOR_ID_, String OPERATOR_NAME_) {
        ...
        omJdbcTemplate.execute("SET FOREIGN_KEY_CHECKS = 0");
        ...
        final List<Map<String, Object>> orgList = omJdbcTemplate.queryForList("with recursive CTE as (select * from OM_ORG where ORGN_SET_ID_ = ? and PARENT_ORG_ID_ is null union all select OM_ORG.* from OM_ORG inner join CTE on OM_ORG.ORGN_SET_ID_ = ? and CTE.ORG_ID_ = OM_ORG.PARENT_ORG_ID_) select * from CTE", BASE_ORGN_SET_ID_, BASE_ORGN_SET_ID_);
        ...
        omJdbcTemplate.execute("SET FOREIGN_KEY_CHECKS = 1");
    ...
    public OrgnChange compareOrgnSet(String BASE_ORGN_SET_ID_, String ORGN_SET_ID_, String OPERATOR_ID_, String OPERATOR_NAME_) {
        ...
        sql = "with recursive CTE as (select * from OM_ORG where ORGN_SET_ID_ = ? and PARENT_ORG_ID_ is null union all select OM_ORG.* from OM_ORG inner join CTE on OM_ORG.ORGN_SET_ID_ = ? and CTE.ORG_ID_ = OM_ORG.PARENT_ORG_ID_) select * from CTE";
        ...
        List<Map<String, Object>> orgList = omJdbcTemplate.queryForList(sql, BASE_ORGN_SET_ID_, BASE_ORGN_SET_ID_);
        List<Map<String, Object>> mirrorServerOrgList = omJdbcTemplate.queryForList(sql, ORGN_SET_ID_, ORGN_SET_ID_);
